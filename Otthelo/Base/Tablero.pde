...
class Tablero {
   ...
   * donde x = jugador1, y = jugador2
   */
  void reconocimentoFichas(){
  
    for(int i= 0; i< 8; i ++){
      for(int j= 0; j < 8; j++){
        if(mundo[i][j] != 0){
          if(mundo[i][j] == 1){
            println("Aqui hay una ficha negra, en " + i + ", " + j );
            checkLaterales(i, j);
          }
          
          if(mundo[i][j] == 2){
            println("Aqui hay una ficha blanca, en " + i + "x, " + j + "y");

          }
        
      } else{
        
         println("Aqui no hay una ficha en " + i + ", " + j );
          }
    ...
    void checkLaterales(int i, int j){
      //izquierda, central
      if(mundo[i-1][j] == 0 && i > 0 && j > 0){
        println("Aqui puedes jugar " + (i-1) + ", " + (j) );
        stroke(0, 0, 0);
        noFill();
        circle((i-1)*tamCasilla+(tamCasilla/2), (j)*tamCasilla+(tamCasilla/2), tamCasilla*3/5);
      ...
      if(mundo[i+1][j] == 0 && i > 0 && j > 0){
        ...
        //arriba, central
        if(mundo[i][j-1] == 0 && i > 0 && j > 0){
        println("Aqui puedes jugar " + (i) + ", " + (j-1) );
        stroke(0, 0, 0);
        noFill();
        circle((i)*tamCasilla+(tamCasilla/2), (j-1)*tamCasilla+(tamCasilla/2), tamCasilla*3/5);         
        ...
        circle((i)*tamCasilla+(tamCasilla/2), (j+1)*tamCasilla+(tamCasilla/2), tamCasilla*3/5);         
         ...
         //esquina inf derecha, central
        if(mundo[i+1][j+1] == 0 && i > 0 && j > 0){
        println("Aqui puedes jugar " + (i+1) + ", " + (j+1) );
        stroke(0, 0, 0);
        noFill();
        circle((i+1)*tamCasilla+(tamCasilla/2), (j+1)*tamCasilla+(tamCasilla/2), tamCasilla*3/5);         
       
       
       }
       
         //esquina supp izq, central
        if(mundo[i-1][j-1] == 0 && i > 0 && j > 0){
        println("Aqui puedes jugar " + (i-1) + ", " + (j-1) );
        stroke(0, 0, 0);
        noFill();
