class Jugador{
  ...
  /**
   * Método principal de acción del jugador
   * @param tbl El tablero actual del juego
   * @return Vector Coordenadas de la jugada
   */
   ...
   /**
   * Método que permite generar los hijos del nodo actual según los movimientos posibles
   * @param n Nodo al que deben agregarse los nodos hijos
   * @param turno Indicador del turno actual
   ...
   */
   ...
   /**
   * Método que permite saber si un nodo es terminal
   * @param n Nodo a verificar
   * @return Indicador de si el nodo es terminal
   */
   ...
   int heuristica(Nodo n, boolean turno)
    {
      ...
      if(this.dificultad == 0) { //Intermedio
        E = X = C = H1 = H2 = 0;
      } else if(this.dificultad == 1) { //Experto
        E = X = C = 0;
      }
      
      int valor =(64*E + 16*X + 16*C + 4*H1 + 16*H2 + p - 4*m);
      return valor;
            
    }
    
  /**
   * Función de evaluación para conocer la mejor tirada
   * @param n Nodo a verificar
   * @param turno Turno actual
   * @return valor del a función
   */
   ...
   * @param n Nodo a verificar
   * @param turno Turno actual
   * @param profund Profundidad de ramificación
   ...
   */
   int minimax(Nodo n, boolean turno, int profund, int alfa, int beta)
   {
     ...
     if(profund >= this.deep || nodoTerminal(n))
        {
   ...
   * @param d Indicador de dificultad
   */
